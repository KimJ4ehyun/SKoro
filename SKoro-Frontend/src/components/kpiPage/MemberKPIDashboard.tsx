...
interface MyDetailKPIItem {
  teamKpiId: number
  teamKpiName: string
  teamKpiGoal: string
  teamKpiAchievementRate: number
  teamKpiWeight: number
  taskId: number
  taskName: string
  taskDetail: string
  taskGoal: string
...
const MemberKPIDashboard: React.FC = () => {
  const [expandedTasks, setExpandedTasks] = useState<Set<number>>(new Set())
  const [expandedDetails, setExpandedDetails] = useState<Set<string>>(new Set())
  const [selectedYear, setSelectedYear] = useState<string>('2025년도')
  const [myDetailKPI, setMyDetailKPI] = useState<MyDetailKPIItem[]>([])

  const myEmpNo = useUserInfoStore((state) => state.empNo)
  const [yearOptions, setYearOptions] = useState<any[]>([])
  useEffect(() => {
    // 팀원의 평가 기간 목록 조회 (연도, 분기 선택할 때 사용)
    TeamService.getMemberPeriods(myEmpNo)
      .then((data) => {
  ...
  // myDetailKPI 데이터를 KPI별로 그룹화
  const processedKPIData: ProcessedKPI[] = React.useMemo(() => {
    const kpiMap = new Map<number, ProcessedKPI>()

    myDetailKPI.forEach((item) => {
      if (!kpiMap.has(item.teamKpiId)) {
        kpiMap.set(item.teamKpiId, {
          teamKpiId: item.teamKpiId,
          teamKpiName: item.teamKpiName,
          teamKpiGoal: item.teamKpiGoal,
          teamKpiAchievementRate: item.teamKpiAchievementRate,
          teamKpiWeight: item.teamKpiWeight,
          teamKpiGrade: item.teamKpiGrade,
          participants: item.participants,
          tasks: [],
        })
      }

      const kpi = kpiMap.get(item.teamKpiId)!
      kpi.tasks.push({
        taskId: item.taskId,
        taskName: item.taskName,
        taskDetail: item.taskDetail,
        taskGoal: item.taskGoal,
        taskWeight: item.taskWeight,
        taskStartDate: item.taskStartDate,
        taskEndDate: item.taskEndDate,
        employee: item.employee,
        taskGrade: item.taskGrade,
        taskAchievementRate: item.taskAchievementRate,
        taskContributionScore: item.taskContributionScore,
      })
    })
  ...
  useEffect(() => {
    TeamService.getMyTeamKpiDetail(
      selectedYear
  ...
  const expandAllTasks = () => {
    const allIds = processedKPIData.map((item) => item.teamKpiId)
  ...
  const getGradeByRate = (rate: number) => {
    if (rate >= 95) return 'S'
    if (rate >= 85) return 'A'
    if (rate >= 75) return 'B'
    if (rate >= 65) return 'C'
  ...
  const renderGradeScale = (grade: any) => (
  ...
  const renderTask = (task: ProcessedKPI['tasks'][0]) => {
    const isDetailsExpanded = expandedDetails.has(`task-${task.taskId}`)
    const currentGrade = getGradeByRate(task.taskAchievementRate)
      ...
      >
              ...
              <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                <div
                  className={`h-2 rounded-full ${getProgressBarColor(
                    task.taskAchievementRate
                  )}`}
                  style={{
                    width: `${
                      task.taskAchievementRate > 100
                        ? 100
                        : task.taskAchievementRate
                    }%`,
                  }}
        ...
        {isDetailsExpanded && (
          <div className="mt-4 bg-gray-50 rounded-lg text-sm">
            ...
            {renderGradeScale(task.taskGrade)}
          </div>
        )}
      </div>
    )
      ...
      >
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                <div
                  className={`h-2 rounded-full ${getProgressBarColor(
                    item.teamKpiAchievementRate
                  )}`}
                  style={{
                    width: `${
                      item.teamKpiAchievementRate > 100
                        ? 100
                        : item.teamKpiAchievementRate
                    }%`,
                  }}
        ...
        <div className="hidden lg:flex lg:items-center lg:justify-between lg:p-4 flex w-full">
          ...
          <div className="flex-[7]">
            <div className="text-sm text-gray-600">{item.teamKpiGoal}</div>
          </div>
          ...
          <div className="flex-[1]">
            <div
              className={`text-sm font-medium rounded-md py-[0.1rem] px-[0.5rem] w-[fit-content]
              ${
                item.teamKpiWeight >= 30
                  ? 'bg-[#FFCFD0] text-[#E74E50]'
                  : item.teamKpiWeight >= 20
                  ? 'bg-[#E4EFFF] text-[#5F87C2]'
                  : item.teamKpiWeight >= 10
                  ? 'bg-[#FFF2CB] text-[#D9A91A]'
                  : 'bg-[#ECECEC] text-[#898989]'
              }`}
            >
          ...
          <div className="px-4 pb-4">
            <div className="bg-gray-50 rounded-lg text-sm">
              ...
              {item.participants && item.participants.length > 0 && (
                <div className="p-3 sm:p-4 pb-0 sm:pb-0 bg-gray-50 rounded-lg">
                  <div className="font-semibold text-gray-700 mb-2">참여자</div>
                  <div className="flex flex-wrap gap-2">
                    {item.participants.map((participant, index) => (
                      ...
                      >
                        <div className="w-6 h-6 bg-gradient-to-br from-gray-400 to-gray-600 rounded-full flex items-center justify-center text-xs font-medium text-white">
                          {participant.empName.charAt(0)}
                        </div>
                        <span className="text-xs text-gray-700">
                          {participant.empName} ({participant.empNo})
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              {renderGradeScale(item.teamKpiGrade)}
            </div>
          </div>
        )}
  ...
  return (
    <div className="w-full px-10 flex-1 min-h-0 flex flex-col mb-4">
      ...
      <div className="mt-3 flex-1 min-h-0 overflow-auto mx-[-0.2rem] px-[0.2rem]">
        {processedKPIData.map(renderKPIItem)}
      </div>
    </div>
  )
}

export default MemberKPIDashboard
